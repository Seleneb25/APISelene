async function getData(recurso) {
    try {
        const res = await fetch(`/rest-api-selene/api/${recurso}`);
        if (!res.ok) {
...
async function createUser(){
    const nuevo = { id: 25, nombre: "Selene García", rol: "Desarrolladora"}; 
    // POST al recurso correcto (tabla 'usuarios' en el API)
    const res = await fetch(`/rest-api-selene/api/usuarios`, {
        method: 'POST',
    ...
    try {
        await fetch('/rest-api-selene/api/logevent', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function sendLogEvent(){
    ...
    try{
        const res = await fetch(`/rest-api-selene/api/logevent`, {
            method: 'POST',
